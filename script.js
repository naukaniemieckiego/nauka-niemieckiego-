...
let currentLesson = 0;
let currentItem = 0;
let points = Number(localStorage.getItem("anna2d_points") || 0);
let badges = Number(localStorage.getItem("anna2d_badges") || 0);
...
function updateStats() {
  ...
  const percent = Math.min((points / 40) * 100, 100);
  levelEl.textContent = level;
  progressText.textContent = `Poziom ${level} · ${Math.round(percent)}% drogi do B1`;
  progressFill.style.width = percent + "%";

  localStorage.setItem("anna2d_points", points);
  localStorage.setItem("anna2d_badges", badges);
}

function speakGerman(text) {
  if (!("speechSynthesis" in window)) {
    result.textContent = "Ta przeglądarka nie obsługuje głosu.";
    return;
  }
  ...
  utterance.onstart = () => teacher.classList.add("speaking");
  utterance.onend = () => teacher.classList.remove("speaking");
  utterance.onerror = () => teacher.classList.remove("speaking");
...
function listenAndCheck() {
  ...
  recognition.onresult = (event) => {
    ...
    if (score >= 0.85) {
      ...
      result.textContent = "⚠️ Prawie dobrze. +1 punkt. Powtórz jeszcze raz.";
